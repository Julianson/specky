...
# Simple drift: compare key terms in constitution vs spec
CONST_TERMS=$(grep -oP '\b[A-Z][a-z]+(?:\s[A-Z][a-z]+)+\b' "$CONST" 2>/dev/null | sort -u | wc -l)
SPEC_TERMS=$(grep -oP '\b[A-Z][a-z]+(?:\s[A-Z][a-z]+)+\b' "$SPEC" 2>/dev/null | sort -u | wc -l)

# Check if constitution scope terms appear in spec
CONST_SCOPE=$(grep -iP '(in scope|out of scope|must|shall not)' "$CONST" 2>/dev/null | wc -l)
SPEC_COVERAGE=$(grep -iP '(in scope|out of scope|must|shall not)' "$SPEC" 2>/dev/null | wc -l)
